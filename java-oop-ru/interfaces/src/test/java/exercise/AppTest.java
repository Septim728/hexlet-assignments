...
class AppTest {
    ...
    void testReversedSequence1() {
        ...
        var reversed = new ReversedSequence(stringBuilder.toString());
        assertThat(reversed.toString()).isEqualTo(stringBuilder.reverse().toString());
    }

    @Test
    void testReversedSequence2() {
        var text = "abcdef";
        var reversed = new ReversedSequence(text);
        assertThat(text.length()).isEqualTo(reversed.length());
    }
    ...
    void testReversedSequence3() {
        var text = "abcdef";
        var index = 2;
        var charInd = text.charAt(index);
        var reversed = new ReversedSequence(text);
        assertThat(text.charAt(index)).isEqualTo(charInd);
    }
    // END
}
